...
namespace Arcade_Monitor
{
	public partial class Form1 : Form
	{
		...
		public Form1()
		{
			...
			try
			{
				string[] gameExes = Directory.GetFiles(directory + "\\games", "*.EXE", SearchOption.AllDirectories);
		...
		private void timer1_Tick(object sender, EventArgs e)
		{
			...
			if (!skip)
			{
				runningGames = processes.Where(c => gameNames.Contains(c.ProcessName)).ToList();
			}
			...
			if (!runningGames.Any(c => c.MainWindowHandle == focusHWND) && runningGames.Count() >= 1 && thisProcess.MainWindowHandle != focusHWND)
			{
